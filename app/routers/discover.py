...
def search(
    ...
    """
    Search users and reels at the same time.
    q = the search query from the search bar
    """
    query = f"%{q}%"  # % is SQL wildcard — means "anything before/after"

    # Search users by username, full name, or school
    users = (
        db.query(User)
        .filter(
            User.is_active == True,
            (
                User.username.ilike(query) |      # ilike = case-insensitive LIKE
